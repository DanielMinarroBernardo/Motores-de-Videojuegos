...
void Cubo::move(double tiempo_trasncurrido) {
	...
	//pos.x = pos.z+ vel.z * tiempo_trasncurrido;
	...
	//// aceleracion 
	//vel.y = vel.y - 9.8/4 * tiempo_trasncurrido;
...
bool Rayo::colision(glm::vec3 Punto) {
	...
	if (result >= 1.0f - epsilon && result <= 1.0f + epsilon) {
		return true;
	}
	else {
		return false;
...
float Raycast::raycast(Cubo cubo, Rayo ray) {
	//Metodo Iluso ( solo me muevo en un eje (x,y,z) )ç
	if (ray.direccion.y != 0) {
		//punto mas cercano
		float distancia = fminf(cubo.pos.y - ray.pos.y, cubo.pos.y + cubo.diagonal.y - ray.pos.y);
		//pos_final -> si me choco sera  la posicion del cubo con el que me choco
		//pos final  = pos inicial + vel * tiempo
		//vel*tiempo = posfinal-posinical = distancia
