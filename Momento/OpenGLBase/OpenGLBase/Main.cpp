...
glm::vec3 pos_camara = glm::vec3(0, 5, 10);
...
int main() {
	...
	PhysicalObject cubo2(CUBO);

	cubo2.addCube(new Cubo(
		{ 0.0f, 5.0f, 0.0f },
		{ 1.0f, 1.0f, 1.0f }
	));

	cubo2.setSpeed(glm::vec3(0, 5.0, 0));
	...
	do {
		...
		// Manera ilusa de resolucion predictiva
	/*	float tiempo_de_impacto = Raycast::raycast(suelo, Ray(cubo.pos, cubo.vel, cubo.vel.y));
		...
		// Movimiento solucionado
		
		cubo.collision(suelo);
		cubo.update(deltaTime);

		cubo.collision(*cubo2.c);
		cubo.update(deltaTime);

		

		cubo2.collision(suelo);		
		cubo2.update(deltaTime);

		cubo2.collision(*cubo.c);
		cubo.update(deltaTime);
